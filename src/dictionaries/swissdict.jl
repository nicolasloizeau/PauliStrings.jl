...
# Swiss dict
# ----------
mutable struct SwissDict{K,V} <: AbstractDictionary{K,V}
...
# Token interface
# ---------------
Dictionaries.istokenizable(::SwissDict) = true
Dictionaries.tokentype(::SwissDict) = Int

Base.@propagate_inbounds function Dictionaries.iteratetoken(indices::SwissDict, index::Int)
...
function _get!(default::Base.Callable, h::SwissDict{K,V}, key::K) where {K,V}
    index, tag = ht_keyindex2!(h, key)
...
Base.in(key, v::Base.KeySet{<:Any,<:SwissDict}) = (ht_keyindex(v.dict, key) > 0)
...
Base.@propagate_inbounds function Base.iterate(v::Union{Base.KeySet{<:Any,<:SwissDict},Base.ValueIterator{<:SwissDict}}, state=v.dict.idxfloor)
