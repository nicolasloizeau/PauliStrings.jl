...
function rk4(H::AbstractOperator, O::AbstractOperator, dt::Real;
             hbar::Real=1, heisenberg=true, M=2^20, trim::Function=trim, f=f_unitary)
    s = heisenberg ? 1im : -1im
    k1 = trim(f(H, O, s, hbar), M)
    k2 = trim(f(H, O + dt * k1 / 2, s, hbar), M)
    k3 = trim(f(H, O + dt * k2 / 2, s, hbar), M)
    k4 = trim(f(H, O + dt * k3, s, hbar), M)
...
function dopri5(H::AbstractOperator, O::AbstractOperator, dt::Real;
    ...
    k1 = trim(f(H, O, s, hbar), M)
    k2 = trim(f(H, O + dt*a21*k1, s, hbar), M)
    k3 = trim(f(H, O + dt*(a31*k1 + a32*k2), s, hbar), M)
    k4 = trim(f(H, O + dt*(a41*k1 + a42*k2 + a43*k3), s, hbar), M)
    k5 = trim(f(H, O + dt*(a51*k1 + a52*k2 + a53*k3 + a54*k4), s, hbar), M)
    k6 = trim(f(H, O + dt*(a61*k1 + a62*k2 + a63*k3 + a64*k4 + a65*k5), s, hbar), M)
...
`M` is the number of strings to keep.
"""
function rk4(H::Function, O::AbstractOperator, dt::Real, t::Real; hbar::Real=1, heisenberg=true, M=2^20, trim::Function=trim, f=f_unitary)
    return rk4(H(t), O, dt; hbar=hbar, heisenberg=heisenberg, M=M, trim=trim, f=f)
end
...
If `heisenberg=false` then it is assumed that O is a density matrix.
"""
function rk4_lindblad(H::AbstractOperator, O::AbstractOperator, dt::Real, L; hbar::Real=1, heisenberg=true, M=2^20, gamma=[], trim::Function=trim)
    @assert length(gamma) == length(L) || length(gamma) == 0
    (length(gamma) == 0) && (gamma = ones(length(L)))
    f(H, O, s, hbar) = f_lindblad(H, O, s, hbar, L, gamma)
    return rk4(H, O, dt; hbar=hbar, heisenberg=heisenberg, M=M, trim=trim, f=f)
end
